...
void writeRegister(int registerAddress, int registerValue){
  ...
  printf("writeRegister tempArray[0]\t%x\n", tempArray[0]);
  printf("writeRegister tempArray[1]\t%x\n", tempArray[1]);
}

void readOneRegister(int registerAddressX){
  ...
  printf("Register %x:\t%x\n", registerAddressX, register_value[0]);
}
...
int8_t bmm150_user_i2c_reg_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    tempArray[0] = reg_addr;
    tempArray[1] = reg_data;

    nrf_drv_twi_tx(&m_twi, 0x10, &tempArray, 2, true);

    printf("tempArray 0:\t%x\n", tempArray[0]);
    printf("tempArray 1:\t%x\n", tempArray[1]);
 ...
 */
 ...
 void customFunction(uint8_t val){
  val = 9;
  printf("Val:\t%x\n", val);
 }
...
{  
    ...
    printf("i2c_reg_read Register Value:\t%x\n", register_value[0]);
