...
int8_t bmm150_user_i2c_reg_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    uint8_t tempArray[] = {reg_addr, reg_data};

    printf("---------------------------------\n");

    printf("user_i2c_reg_write reg_data:\t%x\n", reg_data);

    error_report = nrf_drv_twi_tx(&m_twi, 0x10, reg_addr, 1, false);
    error_report = nrf_drv_twi_tx(&m_twi, 0x10, reg_data, length, false);
...
int8_t bmm150_user_i2c_reg_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    error_report_read2 = nrf_drv_twi_rx(&m_twi, 0x10, register_value, length);

    int sizeOfRegData = sizeof(register_value)/sizeof(register_value[0]);
    printf("Size of read array:\t\t%d\n", length);
    printf("Register Address:\t\t%x\n", reg_addr);

    for(int i = 0; i < length; i++){
    reg_data[i] = register_value[i];
      printf("Register Data[%d]:\t\t%x\n", i, register_value[i]);
    }
