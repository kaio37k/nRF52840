...
int8_t bmm150_user_i2c_reg_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    printf("i2c_reg_write, reg_addr:\t%x\n", tempArray[0]);
    printf("i2c_reg_write, reg_data:\t%x\n", tempArray[1]);

    error_report = nrf_drv_twi_tx(&m_twi, 0x10, &tempArray, length, false);
...
int8_t bmm150_user_i2c_reg_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t length, void *intf_ptr)
{
    ...
    error_report_read1 = nrf_drv_twi_tx(&m_twi, 0x10, &register_addr, 1, false);
    error_report_read2 = nrf_drv_twi_rx(&m_twi, 0x10, reg_data, length);
