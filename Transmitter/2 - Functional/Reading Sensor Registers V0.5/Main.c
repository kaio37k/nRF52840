...
int main(void)
{
    ...
    while(true){
      ...
      //uint8_t addressX[1] = {0x4C};
      uint8_t dataX[1];

      bmm150_get_regs(0x4C, &dataX, 1, &dev);

      //printf("dataX[0]:\t%d\n", dataX[0]);

      //printf("use_i2c_read:\t%d\n", bmm150_custom_i2c_reg_read(0x4c, &dataX, 1, &dev));

      //bmm150_custom_i2c_reg_read(0x4c, &dataX, 1, 0);
      printf("dataX:\t%d\n", dataX[0]);
